...
def xmind_to_zentao_csv_file(xmind_file: str) -> str:
    ...
    with open(zentao_file, 'w', encoding='utf8') as f:
        ...
        logging.info('Convert XMind file(%s) to a zentao csv file(%s) '
                     'successfully!', xmind_file, zentao_file)
...
def gen_a_testcase_row(testcase_dict: Dict[str, Any]) -> List[str]:
    ...
    return [
        case_module, case_title, case_precondition, case_step,
        case_expected_result, case_keyword, case_priority, case_type,
        case_apply_phase
    ]
...
def gen_case_step_and_expected_result(
